...
def SelectiveScanOflex_fn(u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=False, **kwargs):
    """Run selective scan through the official ``mamba_ssm`` implementation."""
    if selective_scan_fn is None:
        raise ImportError("Failed to load selective_scan_fn.")

    # Map tensors to the layout required by mamba_ssm.
    # u: (B, D, L)
    # delta: (B, D, L)
    # A: (D, N)
    ...
    # mamba_ssm requires D to be one-dimensional.
...
class VSSM(nn.Module):
    ...
    def _selective_scan(self, u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=True, **kwargs):
        return SelectiveScanOflex_fn(u, delta, A, B, C, D, delta_bias, delta_softplus)
